...
from keras_efficientnets import EfficientNetB0, BlockArgs
...
def MobilenetConv2D_BN_Relu(kernel, alpha, filters):
    ...
    return compose(Conv2D(last_block_filters, kernel, use_bias=False),
                   BatchNormalization(epsilon=1e-3, momentum=0.99),
                   LeakyReLU(alpha=0.1))


def mobilenetv2_yolo_body(inputs, num_anchors, num_classes, alpha=0.75):
    mobilenetv2 = MobileNetV2(alpha=alpha, input_tensor=inputs, include_top=False,
                              weights='imagenet')  # 'imagenet'
    x = mobilenetv2.output
    x, y1 = make_mobilenet_last_layers(x, alpha, 512, num_anchors * (num_classes + 5))

    x = compose(
        MobilenetConv2D_BN_Relu((1, 1), alpha, 256),
        UpSampling2D(2))(x)
    f2 = mobilenetv2.get_layer('block_12_project_BN').output
    x = Concatenate()([x, f2])
    x, y2 = make_mobilenet_last_layers(x, alpha, 256, num_anchors * (num_classes + 5))

    x = compose(
        MobilenetConv2D_BN_Relu((1, 1), alpha, 128),
        UpSampling2D(2))(x)
    f3 = mobilenetv2.get_layer("block_5_project_BN").output
    x = Concatenate()([x, f3])
    x, y3 = make_mobilenet_last_layers(x, alpha, 128, num_anchors * (num_classes + 5))
...
def mobilenetV1_yolo_body(inputs, num_anchors, num_classes, alpha=0.75):
    mobilenet = MobileNet(input_tensor=inputs, weights='imagenet', alpha=alpha)  # default 'imagenet'
...
def efficient_yolo_body(inputs, num_anchors, num_classes):
    # efficientnet
    efficientnet_modle = EfficientNetB0(input_tensor=inputs, weights='imagenet', include_top=False)
    # input: 416 x 416 x 3
...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    x, y1 = make_last_layers(f1, 256, num_anchors * (num_classes + 5))  # 512
    # x, y1 = make_mobilenet_last_layers(f1, alpha, 512, num_anchors * (num_classes + 5))
