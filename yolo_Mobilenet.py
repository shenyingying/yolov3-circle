...
class YOLO(object):
    _defaults = {
        "model_path": 'logs/MobileNetV2_0.5_224_layerShort/trained_weights_final.h5',
        "anchors_path": 'model_data/yolo_kuo.txt',
    ...
    def __init__(self, **kwargs):
        # self.model_path = 'logs/MobileNetV1_0.75_320/trained_weights_final.h5'  # model path or trained weights path
        # self.anchors_path = 'model_data/yolo_kuo.txt'
        ...
        self.anchors = self._get_anchors()
        self.sess = K.get_session()
        self.model_image_size = (224, 224)  # fixed size or (None, None), hw
        self.boxes, self.scores, self.classes = self.generate()
...
def detect_img(yolo):
    ...
    #  计算整个,算map
    f = open("/home/sy/data/work/StandardCVSXImages/train.txt", 'r')
    try:
        for img in f:
            print(len(img))
            img = img[:len(img) - 1]
            ...
            #     continue
            # else:
            r_image, out_boxes, out_socres = yolo.detect_image(image)
            # r_image.show()
            box = "pupils " + str(out_socres[0]) + " " + str(
                np.floor(out_boxes[0][1] + 0.5).astype('int32')) + " " + str(
                np.floor(out_boxes[0][0] + 0.5).astype('int32')) + " " + str(
                np.floor(out_boxes[0][3] + 0.5).astype('int32')) + " " + str(
                np.floor(out_boxes[0][2] + 0.5).astype('int32'))
            box1 = "pupils " + str(out_socres[0]) + " " + str(
                np.floor(out_boxes[1][1] + 0.5).astype('int32')) + " " + str(
                np.floor(out_boxes[1][0] + 0.5).astype('int32')) + " " + str(
                np.floor(out_boxes[1][3] + 0.5).astype('int32')) + " " + str(
                np.floor(out_boxes[1][2] + 0.5).astype('int32'))
            txt_file.writelines(box + "\n")
            txt_file.writelines(box1)
            txt_file.close()
    finally:
        f.close()
...
def detect_test(yolo, json_name, test_out_json='caltech_new_result_0.001.json',
                data_dst='../caltech_ped/caltech-pedestrian-dataset-converter/'):
    import json
    import time
